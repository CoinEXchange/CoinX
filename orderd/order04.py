...
class OrderDaemon(Daemon):
	...
	while True:
		...
		#log.debug('AUTOCOMMIT ' + str(db.get_autocommit()))
		copy_active_orders = active_orders[:]
		
		###################################################
		# just testing updates to original and copy
		# to test if both update the db
		# as well as
		# testing updates
		for left in active_orders:
			log.debug('LEFT:  ' + left.send_to_address + ' ' + left.source + ' ' + left.target + ' ' + str(left.order_type) + ' ' + str(left.status))
			for right in copy_active_orders:
		...
		left.status = '203'
		right.status = '204'
		log.debug('Modifying status ...')
		log.debug('LEFT:  ' + left.send_to_address + ' ' + left.source + ' ' + left.target + ' ' + str(left.order_type) + ' ' + str(left.status))
		log.debug('RIGHT: ' + right.send_to_address + ' ' + right.source + ' ' + right.target + ' ' + str(right.order_type) + ' ' + str(right.status))
		log.debug('saving changes ...')
		#active_orders.save()
		upd_left = Order.update(status = left.status).where(Order.send_to_address == left.send_to_address)
		upd_right = Order.update(status = right.status).where(Order.send_to_address == right.send_to_address)
		log.debug('sleep ...')
		time.sleep(20)
		log.debug('continue ...')
		upd_left.execute()
