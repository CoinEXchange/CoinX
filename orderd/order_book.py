...
class OrderBook(list):
	...
	def discover_price(self):
		...
		self.left = highest_buy()
		self.right = lowest_sell()
		log.debug('HIGHEST BUY: ' + str(self.left))
		log.debug('LOWEST SELL: ' + str(self.right))
		
		if self.left == self.right:
			self.market_price = self.left
			log.debug('there is a market price based on orders' + str(left))
		elif self.left < self.right:
			self.market_price = (self.left + self.right)/2
